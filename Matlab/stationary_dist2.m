...
%     g = g/g_sum;
%     
%     assert(sum(sum(sum(g .* dadbdz)))==1, "Normalisation error: this isn't actually a density")
    ...
    db_tilde(1) = (b(2)-b(1))/2; 
    db_tilde(I) = (b(I)-b(I-1))/2;
    
    da_tilde = ones(J,1); 
    da_tilde(2:J-1) = (a(3:J) - a(1:J-2))/2; 
    da_tilde(1) = (a(2)-a(1))/2; 
    da_tilde(J) = (a(J)-a(J-1))/2;

    % Q: should the end points of each of these also be divided by 2 (as they are currently)? 
    %  That would be necessary to ensure that the sum of da and db are equal to the range of each support, which is a nice feature to have
    ...
    assert(abs(sum(sum(sum(g .* (AAA.*BBB))))-1)<1e-15, "Normalisation error: this isn't actually a density")
