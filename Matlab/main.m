...
% -------------------------------------------------------------------------
% Solve stationary distribution: returns measure by default; to get denisty, add 'density' as the fourth argument
g  = stationaryDistribution(A, a, b, par);

% to check the stationary distribution-measure: do we get the same measure after t=1000
g_longrun = (speye(Nz*I*J) - A'*(1000))\g(:);
max(abs(g_longrun- g(:)))

% % denisty instead of measure
% [g,dadbdz]  = stationaryDistribution(A, a, b, par, 'density');
% % to check the stationary distribution-density: do we get the same density after t=1000
% g_longrun = (speye(Nz*I*J) - A'*(1000))\(g(:).*dadbdz(:));
% max(abs(g_longrun./dadbdz(:)- g(:)))
